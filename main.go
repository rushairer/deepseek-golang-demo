...
func runDatabaseMigrations(db *sql.DB) (*migrate.Migrate, error) {
	...
	if err != nil { return nil, fmt.Errorf("create migration driver: %w", err) }
	m, err := migrate.NewWithDatabaseInstance("file://migrations", "mysql", driver)
	if err != nil { return nil, fmt.Errorf("create migration instance: %w", err) }
	if err := m.Up(); err != nil && !errors.Is(err, migrate.ErrNoChange) { return nil, fmt.Errorf("run migrations: %w", err) }
	return m, nil
}

func main() {
	cfg, err := config.Load(); if err != nil { log.Fatalf("load configuration: %v", err) }
	db, err := models.NewDB(cfg.DBDSN); if err != nil { log.Fatalf("connect database: %v", err) }; defer db.Close()
	m, err := runDatabaseMigrations(db); if err != nil { log.Fatalf("migrate database: %v", err) }; defer m.Close()
	...
	server := api.NewServer(store, runner, executor, cfg.MaxRequestBodyBytes); server.SetupRoutes(router, cfg.APIAuthToken, cfg.RateLimitPerMinute)
	...
	go func(){log.Printf("server listening on %s with model %s",httpServer.Addr,cfg.DeepSeekModel);if err:=httpServer.ListenAndServe();err!=nil&&!errors.Is(err,http.ErrServerClosed){log.Fatalf("serve HTTP: %v",err)}}()
	stop:=make(chan os.Signal,1);signal.Notify(stop,syscall.SIGINT,syscall.SIGTERM);<-stop
	ctx,cancel:=context.WithTimeout(context.Background(),15*time.Second);defer cancel();if err:=httpServer.Shutdown(ctx);err!=nil{log.Printf("graceful shutdown failed: %v",err)}
}
